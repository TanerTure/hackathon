    ...
    for gate_line in eachline(file)
        ...
        if gate_string == "Rn"
            ...
            if match_ != nothing
                ...
                println("Gate: $gate_string, Qubits: $gate_nums, Args: $args")

                #println("Gate: $gate_string, Arguments: $args[1], $args[2], $args[3]")
            end
        elseif gate_string == "CZ"
            # cz gate has no parameters
            push!(gates, op("CZ", s[gate_nums[1]], s[gate_nums[2]]))
            println("Gate: $gate_string, Qubits: $gate_nums")
...
#gate = op("CX", s[1],s[2])
Psi = apply(gates, Psi_0, cutoff=1e-16)
